...
static void i2c_gpdma_tx_isr_cb(void *resource, drv_event_t event, gpdma_chain_trans_t *next_chain)
{
    ...
    switch (event) {
        ...
        case DRV_GPDMA_EVENT_ABORT:
            drv_event = DRV_EVENT_COMMON_ABORT;
            // In i2c read sequence, both I2C_TX_BUSY and I2C_RX_BUSY are set.
            // When the write phase fails and aborts, the read phase will never start,
            // so I2C_RX_BUSY will never be cleared, and must be released here
            env->busy &= ~I2C_RX_BUSY;
            break;
        default:
            drv_event = DRV_EVENT_COMMON_ERROR;
...
static void i2c_abort(OM_I2C_Type *om_i2c)
{
    ...
    if (resource != NULL) {
        ...
        // Disable read and write interrupts
        om_i2c->INTR_MASK = 0;
        // Disable I2C
        om_i2c->ENABLE = I2C_DISABLE;
        // Clear env
...
void drv_i2c_abort_write(OM_I2C_Type *om_i2c)
{
    i2c_abort(om_i2c);
}

void drv_i2c_abort_read(OM_I2C_Type *om_i2c)
{
    i2c_abort(om_i2c);
}
