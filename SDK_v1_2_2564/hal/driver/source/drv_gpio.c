...
void drv_gpio_uninit(OM_GPIO_Type *om_gpio, gpio_mask_t gpio_mask)
{
    const drv_resource_t *resource;
    gpio_env_t *env;
    ...
    drv_gpio_set_dir(om_gpio, gpio_mask, GPIO_DIR_INPUT);
    OM_CRITICAL_BEGIN();
    // clear gpio using flag
    env->gpio_using &= ~gpio_mask;
    OM_CRITICAL_END();
