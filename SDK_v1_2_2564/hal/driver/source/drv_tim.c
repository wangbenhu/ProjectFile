...
void drv_tim_isr(OM_TIM_Type *om_tim)
{
    ...
    // SR bit0~7 (UIF/CC1~4IF/COMIF/TIF/BIF) are aligned with DIER bit0~7 (UIE/CC1~4IE/COMIE/TIE/BIE);
    // SR bit9~12 (CC1~4OF, overcapture) have no dedicated interrupt enable bit,
    // they are gated by the corresponding CCxIE (map DIER bit1~4 -> SR bit9~12)
    irq_en = (om_tim->DIER & 0xFFU) | ((om_tim->DIER & 0x1EU) << 8);
    status = om_tim->SR & irq_en;
    // SR is rc_w0: clear only the flags being handled
    om_tim->SR = ~status;
