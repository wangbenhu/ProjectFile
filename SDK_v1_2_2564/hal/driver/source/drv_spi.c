...
static void spi_abort(OM_SPI_Type *om_spi)
{
    ...
    register_set(&om_spi->STAT, MASK_2REG(SPI_STAT_TX_EMPTY_INT_EN, 0, SPI_STAT_RX_TRIG_INT_EN,  0));
    om_spi->STAT = SPI_STAT_SPI_INT_MASK;
    // Clear tx/rx FIFOs
    om_spi->CTRL = ctrl_reg | SPI_CTRL_TX_CLR_FIFO_MASK | SPI_CTRL_RX_CLR_FIFO_MASK;
    // Reset state and count/pointer to avoid the ISR reading stale values
    env->state  = SPI_STATE_IDLE;
    env->tx_num = 0U;
    env->rx_num = 0U;
    // Master: release CSn
...
void drv_spi_abort_write(OM_SPI_Type *om_spi)
{
    spi_abort(om_spi);
}
...
void drv_spi_isr(OM_SPI_Type *om_spi)
{
    ...
    // If the SPI is in idle state, return directly
    if (env->state == SPI_STATE_IDLE) return;
    ...
    if ((env->state & SPI_STATE_RX) && (env->rx_buf != NULL)) {
        while ((om_spi->STAT & SPI_STAT_RX_NOT_EMPTY_MASK) && (env->rx_cnt < env->rx_num)) {
