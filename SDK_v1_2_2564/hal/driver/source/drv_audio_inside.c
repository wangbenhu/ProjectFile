...
typedef struct {
    volatile audio_sys_vcc_level_t sys_vcc_level;
...
static audio_env_t audio_env = {
    .sys_vcc_level  = AUDIO_SYS_VCC_LEVEL_HIGH,
...
static void _audio_power_on(void)
{
    ...
    // VCC: fit low level
    if(env->sys_vcc_level == AUDIO_SYS_VCC_LEVEL_LOW) {
        REGW(&OM_AUDIO->CODEC_ANA_CTRL_2, MASK_2REG(AU_ANA_LDO28_MODE, 0, AU_ANA_CTRL_AU_LDO28, 3));
    }
...
void drv_audio_inside_set_sys_vcc_level(audio_sys_vcc_level_t level)
{
    audio_env_t *env = (audio_env_t *)audio_resource.env;
    env->sys_vcc_level = level;
}
