...
const uint8_t audio_v1_descriptor[] = {
    ...
    AUDIO_AS_DESCRIPTOR_INIT(0x02, 0x03, AUDIO_MIC_CHANNELS, AUDIO_MIC_FRAME_SIZE_BYTE, AUDIO_MIC_RESOLUTION_BIT, AUDIO_IN_EP, 0x05, AUDIO_IN_PACKET,
...
void usbd_speaker_enable(bool enable);
void usbd_speaker_out(uint8_t *pdata, uint32_t len);
void usbd_mic_enable(bool enable);
void usbd_mic_in_finish(void);
void usbd_mic_in_data_get(uint8_t *pdata, uint32_t len);

void usbd_audio_open(uint8_t busid, uint8_t intf)
{
    if (intf == 1) {
        rx_flag = 1;
        usbd_speaker_enable(true);
        /* setup first out ep read transfer */
        usbd_ep_start_read(busid, AUDIO_OUT_EP, read_buffer, AUDIO_OUT_PACKET);
        USB_LOG_RAW("Speaker OPEN\r\n");
    } else {
        tx_flag = 1;
        ep_tx_busy_flag = false;
        usbd_mic_enable(true);
        USB_LOG_RAW("Mic OPEN\r\n");
    }
}
...
void usbd_audio_close(uint8_t busid, uint8_t intf)
{
    if (intf == 1) {
        rx_flag = 0;
        usbd_speaker_enable(false);
        USB_LOG_RAW("Speaker CLOSE\r\n");
    } else {
        tx_flag = 0;
        usbd_mic_enable(false);
        ep_tx_busy_flag = false;
        USB_LOG_RAW("Mic CLOSE\r\n");
    }
}
...
void usbd_audio_out_callback(uint8_t busid, uint8_t ep, uint32_t nbytes)
{
    ...
    usbd_speaker_out(read_buffer, AUDIO_OUT_PACKET);
    usbd_ep_start_read(busid, AUDIO_OUT_EP, read_buffer, AUDIO_OUT_PACKET);
}
...
void usbd_mic_in(void)
{
    //USB_LOG_RAW(">");
    ep_tx_busy_flag = true;
    usbd_mic_in_data_get(write_buffer, AUDIO_IN_PACKET);
    usbd_ep_start_write(0, AUDIO_IN_EP, write_buffer, AUDIO_IN_PACKET);
}
