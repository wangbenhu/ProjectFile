...
int data_upStream_handler(char **deviceStr, RESPONSE_TYPES type)
{
    ...
    switch(type)
    {
		...
		case TRACK_REPORT_TYPE_DEVICE_INFO:		//设备信息(V1.6: cmd->deviceInfo, +hardVersion/sn/model)
		{
			...
			cJSON_AddStringToObject(deviceInformationValue, "cmd", "deviceInformation");
...
int dataDownStream_handler(APP_CONTROL_Type controlType,uint16_t dataResourse)
{
	...
	if ((CurrentChargeStatusDataGet() == CURRENT_CHARGE_STATUS_CHARGING || CurrentChargeStatusDataGet() == CURRENT_CHARGE_STATUS_LOW_BATTERY)
	    ...
	    && controlType != CONTROL_CHECK_DEVICE_INFO		/* 鉴权配套: 查询设备信息 */
	    && controlType != CONTROL_SET_TIME_SYNCHRONIZE	/* 鉴权配套: 对时 */
...
//    method = cJSON_GetObjectItem(root, "method");
	...
	if ((CurrentChargeStatusDataGet() == CURRENT_CHARGE_STATUS_CHARGING || CurrentChargeStatusDataGet() == CURRENT_CHARGE_STATUS_LOW_BATTERY)
	    ...
	    && strcmp(itemSid->valuestring, "deviceInformation") != 0		/* 鉴权配套: 查询设备信息 */
	    && strcmp(itemSid->valuestring, "timeSync") != 0		/* 鉴权配套: 对时 */
...
//        ret = 1;	//BLE_SendCustomData(CUSTOM_SEC_DATA, (unsigned char *)buff, len);
	...
	//②查询设备信息
	else if (!strcmp(itemSid->valuestring, "deviceInformation"))
    {
			cJSON *dataResourse = cJSON_GetObjectItem(cjson_buff, "ds");
			cJSON *timestamp = cJSON_GetObjectItem(cjson_buff, "ts");
			cJSON *reqId = cJSON_GetObjectItem(cjson_buff, "reqId");
...
//				b_message_data.reqId = (uint8_t *)reqId->valuestring;
	...
	else if (!strcmp(itemSid->valuestring, "geofenceConfig"))
    {
			...
			if (params != NULL && (strcmp(params->string, "params") == 0)) {
				...
				memset(&s_fence_cfg, 0, sizeof(s_fence_cfg));
				...
				/* 坐标字段在 APP 端是带符号 JSON number (valuedouble, 可为负),
				 *  double 转成定点字符串落盘 (%.14f ) */
				if (fenceS && cJSON_IsArray(fenceS)) {
					...
					cJSON_ArrayForEach(p, fenceS) {
						...
						if (la) {
							...
							snprintf(s_fence_cfg.fence.points[i].latitude,
							         sizeof(s_fence_cfg.fence.points[i].latitude),
							         "%.14f", la->valuedouble);
						}
						if (lo) {
							b_message_data.device_geofence.fenceS[i].lon = lo->valuedouble;
							snprintf(s_fence_cfg.fence.points[i].longitude,
							         sizeof(s_fence_cfg.fence.points[i].longitude),
							         "%.14f", lo->valuedouble);
						}
						i++;
					}
				...
				/* fenceD: 带符号 double 数值转定点字符串 (%.14f) */
				if (fenceD && cJSON_IsArray(fenceD)) {
					...
					cJSON_ArrayForEach(p, fenceD) {
						...
						if (la) {
							...
							snprintf(s_fence_cfg.safe_zone.points[i].latitude,
							         sizeof(s_fence_cfg.safe_zone.points[i].latitude),
							         "%.14f", la->valuedouble);
						}
						if (lo) {
							b_message_data.device_geofence.fenceD[i].lon = lo->valuedouble;
							snprintf(s_fence_cfg.safe_zone.points[i].longitude,
							         sizeof(s_fence_cfg.safe_zone.points[i].longitude),
							         "%.14f", lo->valuedouble);
						}
						i++;
					}
