...
extern void user_define_light_start_flag_clear(void);
#if CUSTOM_STACK_CONTROL
...
static void state_timer_callback(void *argument) 
{
    ...
    ch423_set_output(g_current_led_output);
//	log_debug("state_timer_callback = %d %d\r\n",led_params.is_white_run,led_params.is_red_run);
}
...
//满电全亮4 颗白灯
void led_battery_user_define_start(uint32_t time)
{ 
	led_control(4, _WHITE, true, time*1000);
}
////满电全亮4 颗白灯
void led_battery_user_define_stop(void)
{ 
	led_control(4, _WHITE, false, 0);
}
	...
	if(white_led_control_status != W_LED_STATE_IDLE)
	{
		switch(white_led_control_status)
		{
			...
			case W_LED_STATE_FINDING:
				...
				break;
			case W_LED_STATE_USER:
				led_battery_user_define_stop();
				break;
			default:
				break;						
		}
...
void white_led_start(WhiteLedState mode,uint32_t vlue)
{
	...
	if(mode_tmp != W_LED_STATE_IDLE)
	{
		switch(mode_tmp)
		{
			...
			case W_LED_STATE_FINDING:
				...
				break;
			case W_LED_STATE_USER:
				led_battery_user_define_start(vlue);
				break;
			default:
