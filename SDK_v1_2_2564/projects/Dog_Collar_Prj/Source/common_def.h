...
#endif
typedef enum {
    ASING_STOP_BLE_FLAG_NONE = 0,
    ASING_STOP_BLE_FLAG_START = 1,
    ASING_STOP_BLE_FLAG_END = 2,
} ASING_STOP_BLE_FLAG_T;//asing_stop_ble_flag
typedef enum{
	REPORT_OK = 0,
	REPORT_MOTOR_ERROR,
	REPORT_AUDIO_ERROR,
...
//	TASK_CAT1_DEVICE_STATE_REPORT,  //主动上报设备状态（CAT1触发，已弃用，由COMM定时器接管）
	...
	CERT_PRODUCT_ERROR,
	TASK_LED_USER_START,   	// 用户灯光开启
	TASK_LED_USER_STOP,   	//用户灯光关闭 
...
typedef struct {
    ...
    uint8_t lat_dir;            // 南北纬（N=0, S=1, 无效=2）
    uint8_t lon_dir;            // 东西经（E=0, W=1, 无效=2）
    ...
    uint8_t last_valid_lat_dir;  // 上次有效纬度方向（N=0, S=1, 无效=2）
    uint8_t last_valid_lon_dir;  // 上次有效经度方向（E=0, W=1, 无效=2）
    bool has_last_valid;
} GPS_STATUS_t;
...
typedef enum {
    USER_BOUND_STATE_UNBOUND = 0x00,   // 未绑定 
	USER_BOUND_STATE_SUCCESS = 0x01,   // 绑定成功（可选的业务状态）
    USER_BOUND_STATE_UNKNOWN  = 0xFF,   // 初始化（默认）
} UserBoundState_t;
typedef struct {
...
typedef enum
{
	CURRENT_MODE_STANDARD = 0x01,//标准模式
	CURRENT_MODE_SEARCH_PET = 0x02,//寻宠模式
	CURRENT_MODE_DEFAULT = 0xFF,//默认模式
}CURRENT_MODE_T;
typedef enum
{   
	CURRENT_CHARGE_STATUS_LOW_BATTERY = 0x00,//低电量模式
	CURRENT_CHARGE_STATUS_CHARGING = 0x01,//充电模式
	CURRENT_CHARGE_STATUS_FULL = 0x02,//非充电模式
	CURRENT_CHARGE_STATUS_INVALID = 0xFF,//无效状态
}CURRENT_CHARGE_STATUS_T;//当前充电状态
typedef struct {
    CURRENT_MODE_T currentMode;  // 当前模式 default：0 标准：1 寻宠
	CURRENT_CHARGE_STATUS_T currentChargeStatus;  // 当前充电状态
    uint8_t BAT_Capacity;  //0~100, 电池容量
} COMM_GET_STATUS_t;
// 自定义消息结构体
typedef struct {
