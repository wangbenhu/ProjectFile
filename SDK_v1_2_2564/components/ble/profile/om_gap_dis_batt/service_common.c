 ...
 * TYPEDEFS
 */
 #define BLE_BPS_BATTERY_PRESENT   (1U << 0)
#define BLE_BPS_DISCHARGING       (1U << 1)
#define BLE_BPS_CHARGING          (1U << 2)
#define BLE_BPS_CRITICAL          (1U << 3)

#define BLE_BPS_LEVEL_GOOD        (0U << 4)
#define BLE_BPS_LEVEL_LOW         (1U << 4)
#define BLE_BPS_LEVEL_CRITICAL    (2U << 4)
#define BLE_BPS_LEVEL_UNKNOWN     (3U << 4)
enum {
...
{
    uint8_t ble_status = BLE_BPS_BATTERY_PRESENT;

    switch (charge_status) {
    case CHARGE_STATUS_NO_CHARGE:
        // 未接充电器，设备由电池供电
        ble_status |= BLE_BPS_DISCHARGING | BLE_BPS_LEVEL_UNKNOWN;
        break;

    case CHARGE_STATUS_CHARGING:
        // 正在充电
        ble_status |= BLE_BPS_CHARGING | BLE_BPS_LEVEL_UNKNOWN;
        break;

    case CHARGE_STATUS_FULL:
        // 已充满，通常不再处于 Charging 状态
        ble_status |= BLE_BPS_LEVEL_GOOD;
        break;

    case CHARGE_STATUS_INVALID:
    default:
        // 状态未知；不要错误上报为临界低电
        ble_status |= BLE_BPS_LEVEL_UNKNOWN;
        break;
    }
...
//                batt_level_report();
		...
		// 新增：充电状态CCCD处理
            ...
            if (*evt->gatt.write_req.data) {
				CHARGE_STATUS_T PmStatus_tmp = PM_GetChargeStatus();
				if(PmStatus_tmp == CHARGE_STATUS_CHARGING && PM_GetBatteryCapacity() == 100)//识别为充满
				{
					PmStatus_tmp = CHARGE_STATUS_FULL;
				}
				update_charging_status(PmStatus_tmp);
//                chg_status_report();
            }
        }
        #endif /* SERVICE_BATTARY */
