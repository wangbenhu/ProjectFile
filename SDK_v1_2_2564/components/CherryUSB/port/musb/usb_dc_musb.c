...
int usbd_ep_start_write(uint8_t busid, const uint8_t ep, const uint8_t *data, uint32_t data_len)
{
    ...
    txcsrl = HWREGB(USB_BASE + MUSB_IND_TXCSRL_OFFSET);
    if ((ep_idx != 0) && (txcsrl & USB_TXCSRL1_TXRDY)) {
        res = -3;
    } else if ((ep_idx == 0) && (txcsrl & USB_CSRL0_TXRDY)) {
        res = -3;
    } else {
        ...
        if (data_len == 0) {
            if (ep_idx == 0x00) {
                if (g_musb_udc.setup.wLength == 0) {
                    ...
                    // HW auto handle ZLP when wlength=0
                    HWREGB(USB_BASE + MUSB_IND_TXCSRL_OFFSET) = USB_CSRL0_DATAEND;
                } else {
...
static void handle_ep0(void)
{
    ...
    uint16_t read_count;
    do {
        if((ep0_status & USB_CSRL0_DATAEND) && (usb_ep0_state == USB_EP0_STATE_SETUP)) {
            /* work around the dataend not self-clear bug */
            HWREGB(USB_BASE + MUSB_IND_TXCSRL_OFFSET) &= ~USB_CSRL0_DATAEND;
        }
        ...
        if (ep0_status & USB_CSRL0_STALLED) {
            HWREGB(USB_BASE + MUSB_IND_TXCSRL_OFFSET) &= ~USB_CSRL0_STALLED;
            usb_ep0_state = USB_EP0_STATE_SETUP;
            if ((ep0_status & USB_CSRL0_RXRDY) == 0) {
                return;
            }
        }
        ...
        switch (usb_ep0_state) {
            ...
            case USB_EP0_STATE_IN_DATA:
                if ((ep0_status & USB_CSRL0_TXRDY) == 0) {
                    ...
                    // if not switch to OUT
                    if (usb_ep0_state!=USB_EP0_STATE_IN_DATA && usb_ep0_state!=USB_EP0_STATE_IN_STATUS && usb_ep0_state!=USB_EP0_STATE_IN_ZLP) {
                        HWREGB(USB_BASE + MUSB_IND_TXCSRL_OFFSET) = USB_CSRL0_DATAEND;
                    }
                }
