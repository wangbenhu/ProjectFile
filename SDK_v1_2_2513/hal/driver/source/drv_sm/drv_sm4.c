 ...
 * @defgroup DOC DOC
 * @ingroup  DOCUMENT
 * @brief    SM4 algorithm source file
 * @details  SM4 algorithm source file. Implemeted by software.
 *
 * @version
 * Version 1.0
 *  - Initial release
...
#define DRV_SM4_CCM_DEFINE(SM4_CCM_n, sm4_ccm_n)        \
static sm4_ccm_env_t sm4_ccm_n##_env = {                \
    .sm4_ccm_id = (uint32_t)OM_##SM4_CCM_n,             \
}
 ...
 * CONST & VARIABLES
 */
DRV_SM4_CCM_DEFINE(SM4_CCM_0, sm4_ccm_0);
...
void drv_sm4_encrypt(const sm4_ctx_t *ctx, const unsigned char in[16], unsigned char out[16])
{
...
{
	drv_sm4_set_encrypt_key(&ctx->sm4_ctx, key);
	memset(ctx->iv, 0, 16);
	ctx->ivlen = 0;
}
...
{
	while (datalen) {
		uint32_t ivleft = 16 - ctx->ivlen;
		uint32_t len = datalen < ivleft ? datalen : ivleft;
		gm_memxor(ctx->iv + ctx->ivlen, ctx->iv + ctx->ivlen, data, len);
		ctx->ivlen += len;
		if (ctx->ivlen >= 16) {
			drv_sm4_encrypt(&ctx->sm4_ctx, ctx->iv, ctx->iv);
			ctx->ivlen = 0;
		}
		data += len;
		datalen -= len;
...
/************************************SM4 CCM***********************************/
static sm4_ccm_env_t *sm4_ccm_get_env(uint32_t om_sm4_ccm)
{
    if (om_sm4_ccm == OM_SM4_CCM_0) {
        return &sm4_ccm_0_env;
    }
...
static void length_to_bytes(uint32_t len, uint32_t nbytes, uint8_t *out)
{
...
static void sm4_ctr_n_encrypt(const sm4_ctx_t *sm4_ctx, uint8_t ctr[16], size_t n, const uint8_t *in, size_t inlen, uint8_t *out)
{
	uint8_t block[16];
	size_t len;
	...
	while (inlen) {
		...
		drv_sm4_encrypt(sm4_ctx, ctr, block);
		gm_memxor(out, in, block, len);
		ctr_n_incr(ctr, n);
		in += len;
		out += len;
		inlen -= len;
                               ...
                               uint8_t *tag, uint32_t taglen)
{
    ...
    env = sm4_ccm_get_env(om_sm4_ccm);
    if (env == NULL) {
    ...
    drv_sm4_cbc_mac_init(&env->cbc_ctx, key);
	...
	drv_sm4_cbc_mac_update(&env->cbc_ctx, block, 16);
	...
	if (aad && aadlen) {
		...
		drv_sm4_cbc_mac_update(&env->cbc_ctx, block, alen);
		drv_sm4_cbc_mac_update(&env->cbc_ctx, aad, aadlen);
		if (alen + aadlen % 16) {
			drv_sm4_cbc_mac_update(&env->cbc_ctx, zeros, 16 - (alen + aadlen)%16);
		}
	}
	...
	drv_sm4_encrypt(&env->cbc_ctx.sm4_ctx, ctr, block);

	ctr[15] = 1;
	sm4_ctr_n_encrypt(&env->cbc_ctx.sm4_ctx, ctr, 15 - ivlen, in, inlen, out);

	drv_sm4_cbc_mac_update(&env->cbc_ctx, in, inlen);
	if (inlen % 16) {
		drv_sm4_cbc_mac_update(&env->cbc_ctx, zeros, 16 - inlen % 16);
	}
	drv_sm4_cbc_mac_finish(&env->cbc_ctx, mac);
	gm_memxor(tag, mac, block, taglen);

	memset(&env->cbc_ctx, 0x0, sizeof(env->cbc_ctx));
	                           ...
	                           const uint8_t *tag, uint32_t taglen)
{
	...
	drv_sm4_cbc_mac_update(&env->cbc_ctx, out, inlen); // diff from encrypt
	if (inlen % 16) {
		drv_sm4_cbc_mac_update(&env->cbc_ctx, zeros, 16 - inlen % 16);
	}
	drv_sm4_cbc_mac_finish(&env->cbc_ctx, mac);
	...
	// diff from encrypt
	gm_memxor(mac, mac, block, taglen);
