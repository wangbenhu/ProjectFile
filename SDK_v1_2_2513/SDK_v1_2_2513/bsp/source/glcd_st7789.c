 ...
 * @ingroup  DOCUMENT
 * @brief    glcd driver
 * @details  graphics lcd st7789 driver with 3WM1 2-Lane and SPI mode0
 *
 * @version
 * Version 1.0
 *  - Initial release
...
typedef struct {
    ...
    glcd_cb_t st7789_cb;                        // user callback function
...
static uint8_t glcd_init_seq[] = {
    ...
    0x04, 0xB1, 0x05, 0x3C, 0x3C, GLCD_REG_DELAY(1),

    // Porch Control (B2)
    0x01, 0xB2, GLCD_REG_DELAY(1),

    // Frame Rate Control (B3)
    0x07, 0xB3, 0x05, 0x3C, 0x3C, 0x05, 0x3C, 0x3C, GLCD_REG_DELAY(1),

    // Display Inversion Control (B4)
    0x02, 0xB4, 0x03, GLCD_REG_DELAY(1),

    // LCM Control (C0)
    0x04, 0xC0, 0x28, 0x08, 0x04, GLCD_REG_DELAY(1),

    // ID Setting (C1)
    0x02, 0xC1, 0xC0, GLCD_REG_DELAY(1),

    // VDV and VRH Command Enable (C2)
    0x03, 0xC2, 0x0D, 0x00, GLCD_REG_DELAY(1),

    // VRH Set (C3)
    0x03, 0xC3, 0x8D, 0x2A, GLCD_REG_DELAY(1),

    // VDV Set (C4)
    0x03, 0xC4, 0x8D, 0xEE, GLCD_REG_DELAY(1),

    // VCOM Offset Set (C5)
    0x02, 0xC5, 0x1A, GLCD_REG_DELAY(1),

    // Memory Data Access Control (36)
    0x02, 0x36, 0x12, GLCD_REG_DELAY(1),

    // Positive Voltage Gamma Control (E0)
    0x11, 0xE0, 0x04, 0x22, 0x07, 0x0A, 0x2E, 0x30, 0x25, 0x2A, 0x28, 0x26, 0x2E, 0x3A, 0x00, 0x01, 0x03, 0x13, GLCD_REG_DELAY(1),

    // Negative Voltage Gamma Control (E1)
    0x11, 0xE1, 0x04, 0x16, 0x06, 0x0D, 0x2D, 0x26, 0x23, 0x27, 0x27, 0x25, 0x2D, 0x3B, 0x00, 0x01, 0x04, 0x13, GLCD_REG_DELAY(1),

    // Enable 2-lane mode (E7)
    0x02, 0xE7, 0x10, GLCD_REG_DELAY(1),

    // Interface Pixel Format (3A) - RGB565
 ...
 * LOCAL FUNCTIONS
 */
static void st7789_callback(glcd_cb_t* cb);
...
static void glcd_raw_init(uint8_t clk_div)
{
    lcd_config_t lcd_config = {
        ...
        .data_mode = LCD_DATA_MODE_3WM1_PARALLEL,
    };
    drv_lcd_init(&lcd_config);
    drv_lcd_register_isr_callback((drv_isr_callback_t)st7789_callback);
}
...
static void glcd_write_reg(uint8_t reg, const uint8_t *param, uint16_t param_bytes)
{
    ...
    drv_lcd_control(LCD_CONTROL_DATA_2_LANE_EN, (void *)0);
    ...
    drv_lcd_control(LCD_CONTROL_DATA_2_LANE_EN, (void *)1);
    drv_lcd_control(LCD_CONTROL_SET_DATA_BIT, (void *)2);
...
//The instruction issuance and return of the read register command of st7789 are in QSPI 1-line 8bit mode
...
static void st7789_callback(glcd_cb_t* cb)
{
    ...
    if(glcd_env.display_number_bytes > 0) {   // still have data to transfer
        ...
        if(glcd_env.st7789_cb) {          // run st7789 call back if registered
            glcd_env.st7789_cb();
        }
 ...
 * PUBLIC FUNCTIONS
 */
//Although the st7789 chip supports this instruction, this LCD module does not support it.There is also no support for reading back LCD screen data.
