...
/* llx add 其他中断不映射到INT1
	...
	OM_LOG_INF("config_int1 read INT_SOURCE0 0x%02x %02x\r\n", data[0], data[1]);
	...
	OM_LOG_INF("config_int1 write INT_SOURCE0 0x%02x %02x\r\n", data[0], data[1]);

	status |= inv_imu_read_reg(s, INT_SOURCE6_MREG1, 1, &data[0]);
	OM_LOG_INF("config_int1 read INT_SOURCE6_MREG1 0x%02x %02x\r\n", data[0], data[1]);
	...
	OM_LOG_INF("config_int1 write INT_SOURCE6_MREG1 0x%02x %02x\r\n", data[0], data[1]);
*/
	return status;
}
...
//	OM_LOG_INF("INT_CONFIG value1 = 0x%02x\r\n", value); 
