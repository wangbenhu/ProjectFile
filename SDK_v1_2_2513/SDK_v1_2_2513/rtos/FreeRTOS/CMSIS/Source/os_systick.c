...
__WEAK int32_t OS_Tick_Setup (uint32_t freq, IRQHandler_t handler) {
  ...
  load = (configCPU_CLOCK_HZ / freq) - 1U;
  if (load > 0x00FFFFFFU) {
    return (-1);
  }
...
extern uint32_t ulTickGetUpdateCnt(void);
extern pmu_timer_trig_t xPortTimerGet(void);

// Get OS Tick interval.
// extern volatile uint32_t ulTimerUpdateCnt;
__WEAK uint32_t OS_Tick_GetInterval (void) {
  return (configTICK_PERIOD);
}

// Get OS Tick count value.
__WEAK uint32_t OS_Tick_GetCount (void) {
  return  ((drv_pmu_timer_cnt_get() - ulTickGetUpdateCnt()) % configTICK_PERIOD);
}

// Get OS Tick overflow status.
__WEAK uint32_t OS_Tick_GetOverflow (void) {
  pmu_timer_trig_t OM_PMU_TIMER = xPortTimerGet();
  return ((drv_pmu_timer_control(OM_PMU_TIMER, PMU_TIMER_CONTROL_GET_OVERFLOW, NULL) ? 1U : 0U));
}
