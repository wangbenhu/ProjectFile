...
static void usbd_event_handler(uint8_t busid, uint8_t event)
{
    switch (event) {
        ...
        case USBD_EVENT_CONFIGURED:
            ...
            // NOTE: In CDC ACM mode, the length of the data read must be equal to one MPS (64bytes).
...
{
    uint8_t busid = 0;
    uintptr_t reg_base = (uintptr_t)OM_USB;
...
void usbd_cdc_acm_data_send(const char *pdata, uint32_t length)
{
    ...
    length = MIN(length, CDC_MAX_MPS);
    memcpy(write_buffer, pdata, length);

    ep_tx_busy_flag = true;
    usbd_ep_start_write(busid, CDC_IN_EP, write_buffer, length);
}
