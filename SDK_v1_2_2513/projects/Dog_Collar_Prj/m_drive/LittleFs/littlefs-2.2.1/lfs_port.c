...
1.需要可扩展设备型号读/写接口
2.音频文件需要按照ID.wav，设置音频的时候需要下发ID和index（更新/覆盖），播放音频的时候下发index，需要通过ID获取对应index
3.确认wifi读写接口是否ok？WiFi写入文件系统接口，传参：ssid（字符串 ）/mac（字符串）/lat（字符串）/lon（字符串）；
4.新建配置表包含：wifi是否配置、寻宠周期/常规周期/GPS开关周期/围栏告警周期/震动时间/运动参数（结构体的形式写入和读取，文件保存）
...
static const field_system_header_map_t field_system_header_map[] = {
    { SYS_DEVICE_SN_ID,       	"[DEVICE_SN]"        },//设备序列号
    { SYS_FIRMWARE_LTE_VER_ID,  "[LTE_VER]"    		 },//LTE固件版本
    { SYS_FIRMWARE_GNSS_VER_ID, "[GNSS_VER]"    	 },//GNSS固件版本
    { SYS_MANUFACTURE_DATE_ID,	"[MANUFACTURE_DATE]" },//生产日期
    { SYS_BATCH_NUMBER_ID,    	"[BATCH_NUMBER]"     },//批次号
    { SYS_LAST_BOOT_TIME_ID,  	"[LAST_BOOT_TIME]"   },//上次启动时间
    { SYS_TOTAL_RUNTIME_ID,   	"[TOTAL_RUNTIME]"    },//总运行时间
    { SYS_DEVICE_TYPE_ID, 	    "[DEVICE_TYPE]"   },//设备类型
};
/* 测试信息字段头映射表 */
...
int lfs_write_file(const char *path, const void *data, size_t size, int overwrite)
{
    ...
    ret = lfs_mkdir(&lfs, AUDIO_USER_DIR);
...
{
    LFS_LOG_DEBUG("user_bindinfo_set = %d\r\n",is_lfs_mounted());
...
static int wifi_storage_lock(void *context)
{
    (void)context;
    return osMutexAcquire(lfs_mutex, LFS_MUTEX_TIMEOUT_MS) == osOK ? 0 : -1;
}

static void wifi_storage_unlock(void *context)
{
    (void)context;
    osMutexRelease(lfs_mutex);
}

static int wifi_storage_ready_locked(void *context)
{
    (void)context;
#if (LFS_INIT_IN_TASK_LATER)
    uint32_t flags = osEventFlagsWait(sys_events,
                                      EVENT_LFS_READY | EXFLASH_INIT_READY,
                                      osFlagsWaitAll | osFlagsNoClear,
                                      0U);
    return (flags & osFlagsError) == 0U ? 0 : -1;
#else
    return 0;
#endif
}

static int wifi_storage_replace_locked(void *context,
                                       const char *content,
                                       size_t content_len)
{
    const char *temp_file = "/system/wifi.tmp";
    const char *target_file = "/system/wifi.txt";
    struct lfs_info info;
    lfs_file_t wifi_file;
    int result;
    int close_result;
    ...
    result = lfs_file_write(&lfs, &wifi_file, content, content_len);
    if (result < 0 || (size_t)result != content_len) {
        lfs_file_close(&lfs, &wifi_file);
        lfs_remove(&lfs, temp_file);
        return -3;
    }

    result = lfs_file_sync(&lfs, &wifi_file);
    close_result = lfs_file_close(&lfs, &wifi_file);
    if (result < 0 || close_result < 0) {
        lfs_remove(&lfs, temp_file);
        return -4;
    }

    result = lfs_stat(&lfs, temp_file, &info);
    if (result < 0 || info.size != content_len) {
        lfs_remove(&lfs, temp_file);
        return -5;
    }

    result = lfs_rename(&lfs, temp_file, target_file);
    if (result < 0) {
        lfs_remove(&lfs, temp_file);
        return -6;
    }
...
static int wifi_storage_read_locked(void *context,
                                    char *buffer,
                                    size_t buffer_size,
                                    size_t *content_len)
{
    const char *filename = "/system/wifi.txt";
    struct lfs_info info;
    lfs_file_t wifi_file;
    int result;
    int close_result;
    ...
    (void)context;
    if (!buffer || !content_len || buffer_size == 0) {
        return -1;
    }

    result = lfs_stat(&lfs, filename, &info);
    if (result < 0 || info.size == 0 || info.size >= buffer_size) {
        return -2;
    }

    result = lfs_file_open(&lfs, &wifi_file, filename, LFS_O_RDONLY);
    if (result < 0) {
        return -3;
    }
    result = lfs_file_read(&lfs, &wifi_file, buffer, info.size);
    close_result = lfs_file_close(&lfs, &wifi_file);
    if (result < 0 || (lfs_size_t)result != info.size || close_result < 0) {
        return -4;
    }

    buffer[result] = '\0';
    *content_len = (size_t)result;
    return 0;
}

static const WifiConfigStorageOps wifi_storage_ops = {
    .context = NULL,
    .lock = wifi_storage_lock,
    .unlock = wifi_storage_unlock,
    .ready_locked = wifi_storage_ready_locked,
    .replace_locked = wifi_storage_replace_locked,
    .read_locked = wifi_storage_read_locked
};

static int wifi_config_prepare_storage(void)
{
    if (!is_lfs_mounted() && lfs_mount_safe() != 0) {
        LFS_LOG_DEBUG("[LFS][WIFI] mount failed\r\n");
        return -10;
    }
    ...
    int32_t result = osEventFlagsWait(sys_events,
                                      ...
                                      LFS_WAIT_TIMEOUT_MS);
    if (result < 0) {
        LFS_LOG_DEBUG("[LFS][WIFI] wait ready failed: %d\r\n", result);
        return -11;
    }
#endif
    return 0;
...
int wifi_config_set(const WifiConfig_t *config)
{
    ...
    result = wifi_config_prepare_storage();
    if (result < 0) {
        return result;
    }
    result = wifi_config_storage_set(&wifi_storage_ops, config);
    if (result < 0) {
        LFS_LOG_DEBUG("[LFS][WIFI] set failed: %d\r\n", result);
        return result;
    }
    return 0;
}
...
int wifi_config_get(WifiConfig_t *config)
{
    ...
    result = wifi_config_storage_get(&wifi_storage_ops, config);
    if (result < 0) {
        LFS_LOG_DEBUG("[LFS][WIFI] get failed: %d\r\n", result);
        return result;
    }
    return 0;
}

static const char fence_storage_target_path[] = "/system/fence.txt";
static const char fence_storage_temp_path[] = "/system/fence.tmp";
static const char config_storage_target_path[] = "/system/config.txt";
static const char config_storage_temp_path[] = "/system/config.tmp";

typedef struct {
    lfs_file_t file;
} ConfigReplaceContext;

static int config_replace_remove(void *context, const char *path)
{
    (void)context;
    return lfs_remove(&lfs, path);
}

static int config_replace_open(void *context, const char *path)
{
    ConfigReplaceContext *replace_context = (ConfigReplaceContext *)context;

    return lfs_file_open(&lfs,
                         &replace_context->file,
                         path,
                         LFS_O_WRONLY | LFS_O_CREAT | LFS_O_TRUNC);
}

static int config_replace_write(void *context,
                               const char *content,
                               size_t content_len)
{
    ConfigReplaceContext *replace_context = (ConfigReplaceContext *)context;

    return (int)lfs_file_write(&lfs,
                               &replace_context->file,
                               content,
                               content_len);
}

static int config_replace_sync(void *context)
{
    ConfigReplaceContext *replace_context = (ConfigReplaceContext *)context;

    return lfs_file_sync(&lfs, &replace_context->file);
}

static int config_replace_close(void *context)
{
    ConfigReplaceContext *replace_context = (ConfigReplaceContext *)context;

    return lfs_file_close(&lfs, &replace_context->file);
}

static int config_replace_stat(void *context,
                              const char *path,
                              int *type,
                              size_t *size)
{
...
static int config_replace_rename(void *context,
                                const char *old_path,
                                const char *new_path)
{
    (void)context;
...
static int config_storage_replace_locked(void *context,
                                        const char *content,
                                        size_t content_len)
{
    ConfigReplaceContext replace_context = {0};
    TempFileReplaceOps replace_ops = {
        .context = &replace_context,
        .not_found_error = LFS_ERR_NOENT,
        .regular_file_type = LFS_TYPE_REG,
        .remove = config_replace_remove,
        .open = config_replace_open,
        .write = config_replace_write,
        .sync = config_replace_sync,
        .close = config_replace_close,
        .stat = config_replace_stat,
        .rename = config_replace_rename
    };

    (void)context;
    return temp_file_replace(&replace_ops,
                             config_storage_temp_path,
                             config_storage_target_path,
                             content,
                             content_len);
}

static int config_storage_read_locked(void *context,
                                     char *buffer,
                                     size_t buffer_size,
                                     size_t *content_len)
{
    ...
    (void)context;
    if (!buffer || !content_len || buffer_size == 0U) {
        return -1;
    }
    buffer[0] = '\0';
    *content_len = 0U;

    result = lfs_stat(&lfs, config_storage_target_path, &info);
    if (result < 0 || info.type != LFS_TYPE_REG || info.size == 0U ||
    ...
    result = lfs_file_read(&lfs, &config_file, buffer, info.size);
    close_result = lfs_file_close(&lfs, &config_file);
    if (result < 0 || (lfs_size_t)result != info.size || close_result < 0) {
        buffer[0] = '\0';
        return -4;
    }
...
static int fence_storage_replace_locked(void *context,
                                        const char *content,
                                        size_t content_len)
{
    ConfigReplaceContext replace_context = {0};
    ...
    (void)context;
    return temp_file_replace(&replace_ops,
                             fence_storage_temp_path,
                             fence_storage_target_path,
                             content,
                             content_len);
}

static int fence_storage_read_locked(void *context,
                                     char *buffer,
                                     size_t buffer_size,
                                     size_t *content_len)
{
    struct lfs_info info;
    lfs_file_t fence_file;
    int result;
    int close_result;
    ...
    *content_len = 0U;
    ...
    result = lfs_stat(&lfs, fence_storage_target_path, &info);
    if (result < 0 || info.type != LFS_TYPE_REG || info.size == 0U ||
        info.size >= buffer_size) {
        return -2;
    }
    ...
    result = lfs_file_read(&lfs, &fence_file, buffer, info.size);
    close_result = lfs_file_close(&lfs, &fence_file);
    if (result < 0 || (lfs_size_t)result != info.size || close_result < 0) {
        buffer[0] = '\0';
        return -4;
    }
...
static const FenceConfigStorageOps fence_config_storage_ops = {
    .context = NULL,
    .lock = wifi_storage_lock,
    .unlock = wifi_storage_unlock,
    .ready_locked = wifi_storage_ready_locked,
    .replace_locked = fence_storage_replace_locked,
    .read_locked = fence_storage_read_locked
};

static int config_storage_has_data_locked(void *context, int *has_data)
{
    struct lfs_info info;
    int result;
    ...
    *has_data = info.size > 0U ? 1 : 0;
    return 0;
}

static const ConfigTableStorageOps config_table_storage_ops = {
    .context = NULL,
    .lock = wifi_storage_lock,
    .unlock = wifi_storage_unlock,
    .ready_locked = wifi_storage_ready_locked,
    .replace_locked = config_storage_replace_locked,
    .read_locked = config_storage_read_locked,
    .has_data_locked = config_storage_has_data_locked
};

static int config_table_prepare_storage(void)
{
#if (LFS_INIT_IN_TASK_LATER)
    int32_t result = osEventFlagsWait(sys_events,
                                      EVENT_LFS_READY | EXFLASH_INIT_READY,
                                      ...
                                      LFS_WAIT_TIMEOUT_MS);
    if (result < 0) {
        LFS_LOG_DEBUG("[LFS][FENCE] wait ready failed: %d\r\n", result);
        return -11;
    }
    if (!is_lfs_mounted()) {
     //   LFS_LOG_DEBUG("[LFS][FENCE] filesystem not mounted after ready\r\n");
        return -10;
    }
#else
    if (!is_lfs_mounted() && lfs_mount_safe() != 0) {
        LFS_LOG_DEBUG("[LFS][FENCE] mount failed\r\n");
        return -10;
    }
#endif
    return 0;
...
{
    int result = config_table_prepare_storage();

    if (result < 0) {
        return result;
    }
    result = config_table_storage_set(&config_table_storage_ops, config);
    if (result < 0) {
        LFS_LOG_DEBUG("[LFS][FENCE] set failed: %d\r\n", result);
        return result;
    }
    return 0;
}
...
int config_table_get(ConfigTable_t *config)
{
    ...
    result = config_table_prepare_storage();
    if (result < 0) {
        return result;
    }
    result = config_table_storage_get(&config_table_storage_ops, config);
    if (result < 0) {
        memset(config, 0, sizeof(*config));
        LFS_LOG_DEBUG("[LFS][FENCE] get failed: %d\r\n", result);
        return result;
    }
    return 0;
}
...
int fence_config_set(const FenceConfig_t *config)
{
    ...
    if (result < 0) {
        LFS_LOG_DEBUG("[LFS][FENCE_DATA] set failed: %d\r\n", result);
        return result;
    }
    result = fence_config_storage_set(&fence_config_storage_ops, config);
    if (result < 0) {
        LFS_LOG_DEBUG("[LFS][FENCE_DATA] set failed: %d\r\n", result);
        return result;
    }
...
int fence_config_clear(FenceClearTarget_t target)
{
    ...
    if (target != FENCE_CLEAR_TARGET_FENCE &&
        target != FENCE_CLEAR_TARGET_SAFE_ZONE &&
        target != FENCE_CLEAR_TARGET_ALL) {
        return -1;
    }
    memset(&config, 0, sizeof(config));
    config.fence_id = 1U;
    result = fence_config_get(&config);
    if (result < 0) {
        return result;
    }
    if (target == FENCE_CLEAR_TARGET_FENCE ||
        target == FENCE_CLEAR_TARGET_ALL) {
        config.fence.count = 0U;
    }
    if (target == FENCE_CLEAR_TARGET_SAFE_ZONE ||
        target == FENCE_CLEAR_TARGET_ALL) {
        config.safe_zone.count = 0U;
    }
    return fence_config_set(&config);
}
...
int fence_config_get(FenceConfig_t *config)
{
    ...
    result = config_table_prepare_storage();
    if (result < 0) {
        LFS_LOG_DEBUG("[LFS][FENCE_DATA] get failed: %d\r\n", result);
        return result;
    }
    result = fence_config_storage_get(&fence_config_storage_ops, config);
    if (result < 0) {
        memset(config, 0, sizeof(*config));
        LFS_LOG_DEBUG("[LFS][FENCE_DATA] get failed: %d\r\n", result);
        return result;
...
int config_table_init_if_empty(const ConfigTable_t *default_config)
{
    ...
    result = config_table_storage_init_if_empty(&config_table_storage_ops,
                                                default_config);
    if (result < 0) {
        LFS_LOG_DEBUG("[LFS][FENCE] init failed: %d\r\n", result);
    }
    return result;
}
...
int config_table_init_defaults(void)
{
    ...
    return config_table_init_if_empty(&defaults);
}
 ...
 * 初始化小文件同步事件
 */
osStatus_t lfs_port_sync_init(void)
{
	#if (LFS_INIT_IN_TASK_LATER)
    if (sys_events != NULL)
    {
	...
	#endif
    return (sys_events != NULL) ? osOK : osErrorNoMemory;
...
int wav_get_file_size_from_iflash(uint32_t flash_addr, uint32_t *file_size)
{
	...
	if (drv_flash_read(OM_FLASH0, flash_addr, header, sizeof(header)) != 0)
        return -2;
...
/* 音频文件访问适配层：先加锁，再检查文件系统就绪状态。 */
typedef struct {
    lfs_file_t file;
} AudioLfsContext;

static AudioLfsContext audio_lfs_context;

static int audio_lfs_lock(void *context)
{
    (void)context;
    return osMutexAcquire(lfs_mutex, LFS_MUTEX_TIMEOUT_MS) == osOK ? 0 : -1;
}

static void audio_lfs_unlock(void *context)
{
    (void)context;
    osMutexRelease(lfs_mutex);
}

static int audio_lfs_ready_locked(void *context)
{
    (void)context;
#if (LFS_INIT_IN_TASK_LATER)
    uint32_t flags = osEventFlagsWait(sys_events,
...
static int audio_lfs_stat_locked(void *context,
                                 const char *path,
                                 uint32_t *size,
                                 int *is_regular)
{
    ...
    *size = info.size;
    *is_regular = info.type == LFS_TYPE_REG;
    return 0;
}

static int audio_lfs_mkdir_locked(void *context)
{
    int result;

    (void)context;
    result = lfs_mkdir(&lfs, AUDIO_USER_DIR);
    return result == LFS_ERR_EXIST ? 0 : result;
}

static int audio_lfs_open_locked(void *context,
                                 const char *path,
                                 AudioStorageWriteMode mode)
{
    AudioLfsContext *audio_context = (AudioLfsContext *)context;
    int flags = LFS_O_WRONLY | LFS_O_CREAT;

    flags |= mode == AUDIO_STORAGE_WRITE_APPEND ? LFS_O_APPEND : LFS_O_TRUNC;
    return lfs_file_open(&lfs, &audio_context->file, path, flags);
}

static int audio_lfs_write_locked(void *context,
                                  const uint8_t *data,
                                  size_t len)
{
    AudioLfsContext *audio_context = (AudioLfsContext *)context;

    return lfs_file_write(&lfs, &audio_context->file, data, len);
}

static int audio_lfs_sync_locked(void *context)
{
    AudioLfsContext *audio_context = (AudioLfsContext *)context;

    return lfs_file_sync(&lfs, &audio_context->file);
}

static int audio_lfs_close_locked(void *context)
{
    AudioLfsContext *audio_context = (AudioLfsContext *)context;

    return lfs_file_close(&lfs, &audio_context->file);
}

static int audio_lfs_remove_locked(void *context, const char *path)
{
    (void)context;
    return lfs_remove(&lfs, path);
}

static const AudioStorageOps audio_storage_ops = {
    .context = &audio_lfs_context,
    .lock = audio_lfs_lock,
    .unlock = audio_lfs_unlock,
    .ready_locked = audio_lfs_ready_locked,
    .stat_locked = audio_lfs_stat_locked,
    .mkdir_locked = audio_lfs_mkdir_locked,
    .open_locked = audio_lfs_open_locked,
    .write_locked = audio_lfs_write_locked,
    .sync_locked = audio_lfs_sync_locked,
    .close_locked = audio_lfs_close_locked,
    .remove_locked = audio_lfs_remove_locked
};

bool lfs_audio_exist(const char *filename)
{
    return audio_storage_exist(&audio_storage_ops, filename);
}
/* 根据文件名获取音频长度。 */
int lfs_get_wav_size_by_filename(const char *filename, uint32_t *size)
{
    return audio_storage_get_size(&audio_storage_ops, filename, size);
}
/* 根据文件名写入音频。 */
...
{
    AudioStorageWriteMode storage_mode;

    if (mode == AUDIO_WRITE_OVERWRITE) {
        storage_mode = AUDIO_STORAGE_WRITE_OVERWRITE;
    } else if (mode == AUDIO_WRITE_APPEND) {
        storage_mode = AUDIO_STORAGE_WRITE_APPEND;
    } else {
        return -3;
    }
    return audio_storage_write(&audio_storage_ops,
                               filename,
                               data,
                               len,
                               storage_mode);
}
static int audio_flash_get_size(void *context,
                                uint32_t address,
                                uint32_t *size)
{
    (void)context;
    return wav_get_file_size_from_iflash(address, size);
}

static int audio_flash_read(void *context,
                            uint32_t address,
                            uint8_t *buffer,
                            size_t len)
{
    (void)context;
    return drv_flash_read(OM_FLASH0, address, buffer, len) == 0 ? 0 : -1;
}

static int audio_flash_write(void *context,
                             const char *filename,
                             const uint8_t *data,
                             size_t len,
                             AudioStorageWriteMode mode)
{
    audio_write_mode_t write_mode;

    (void)context;
    write_mode = mode == AUDIO_STORAGE_WRITE_APPEND
                     ? AUDIO_WRITE_APPEND
                     : AUDIO_WRITE_OVERWRITE;
    return lfs_audio_write_by_filename(filename, data, len, write_mode);
}

static const AudioFlashCopyOps audio_flash_copy_ops = {
    .context = NULL,
    .get_size = audio_flash_get_size,
    .read = audio_flash_read,
    .write = audio_flash_write
};

int flash_copy_wav_to_lfs(uint32_t flash_addr, const char *filename)
{
    return audio_storage_copy_from_flash(&audio_flash_copy_ops,
                                         flash_addr,
                                         filename);
}
/* 根据文件名删除音频。 */
...
{
    return audio_storage_delete(&audio_storage_ops, filename);
}
/**************************************************************************************/
////////////////////////////////////////////////
