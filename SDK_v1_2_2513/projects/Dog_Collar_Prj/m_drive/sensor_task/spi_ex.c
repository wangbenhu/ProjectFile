...
#define MAX_SPI_SIZE		40
static uint8_t rx_buffer[MAX_SPI_SIZE] = {0};
static uint8_t tx_buffer[MAX_SPI_SIZE] = {0};
...
{
		if(length <= 0 || length > MAX_SPI_SIZE)
		{
				log_debug("********* error spi write length = %d, expect: 0 < length < %d !!!\r\n", length, MAX_SPI_SIZE);
				return -1;
		}
		uint8_t re = 0xff;
		memset(tx_buffer, 0, MAX_SPI_SIZE);
...
int spi_read(uint8_t reg, uint8_t* rx_data, uint32_t length)
{
		...
		uint8_t re = 0xff;
		memset(rx_buffer, 0, MAX_SPI_SIZE);
		memset(tx_buffer, 0, MAX_SPI_SIZE);
