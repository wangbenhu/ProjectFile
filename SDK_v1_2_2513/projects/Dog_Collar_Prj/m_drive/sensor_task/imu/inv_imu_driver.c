...
#define inv_imu_get_time_us()   (inv_imu_get_time_ms() * 1000)
...
int inv_imu_device_reset(struct inv_imu_device *s)
{
	...
	status |= inv_imu_read_reg(s, INV_INT_STATUS, 1, &data);
...
//			inv_imu_sleep_us(accel_odr_us - 200);
			uint32_t wait_time_ms = (accel_odr_us - 200)/1000 >= 1 ? (accel_odr_us - 200)/1000+1 : 1;
			inv_imu_sleep_ms(wait_time_ms);
		}
	...
	if (accel_mode == PWR_MGMT0_ACCEL_MODE_OFF && s->fifo_is_used) {
		 ...
		 */
		s->accel_start_time_us = inv_imu_get_time_ms()*1000;  //= inv_imu_get_time_us();
	}
...
int inv_imu_enable_accel_low_noise_mode(struct inv_imu_device *s)
{
	...
	if (accel_mode == PWR_MGMT0_ACCEL_MODE_OFF && s->fifo_is_used) {
		 ...
		 * about 20ms after enable.
		 */
		s->accel_start_time_us = inv_imu_get_time_ms();
	}
...
int inv_imu_set_accel_frequency(struct inv_imu_device *s, const ACCEL_CONFIG0_ODR_t frequency)
{
	...
	data &= ~ACCEL_CONFIG0_ACCEL_ODR_MASK; //llx add ±2g
	data |= frequency;
...
int inv_imu_enable_gyro_low_noise_mode(struct inv_imu_device *s)
{
	...
	if ((accel_mode == PWR_MGMT0_ACCEL_MODE_LP) &&
		...
		//inv_imu_sleep_us(accel_odr_us);
				inv_imu_sleep_ms((accel_odr_us + 999) / 1000);
	}
...
int inv_imu_disable_gyro(struct inv_imu_device *s)
{
	...
	if ((accel_mode == PWR_MGMT0_ACCEL_MODE_OFF) &&
		...
		fifo_cfg_6_reg |= ((1 & FIFO_CONFIG6_RCOSC_REQ_ON_FIFO_THS_DIS_MASK)
		                   << FIFO_CONFIG6_RCOSC_REQ_ON_FIFO_THS_DIS_POS);
		status |= inv_imu_write_reg(s, FIFO_CONFIG6_MREG1, 1, &fifo_cfg_6_reg);
inv_imu_sleep_ms((gyro_odr_us + 999) / 1000);
	}
...
inv_imu_sleep_ms((2 * accel_odr_us + 999) / 1000);
...
{
	int     status = 0;  
// llx add 其他中断不映射到INT1
	uint8_t data[2];

	status |= inv_imu_read_reg(s, INT_SOURCE0, 2, &data[0]);
	log_debug("config_int1 read INT_SOURCE0 0x%02x %02x\r\n", data[0], data[1]);
	...
	log_debug("config_int1 write INT_SOURCE0 0x%02x %02x\r\n", data[0], data[1]);

	status |= inv_imu_read_reg(s, INT_SOURCE6_MREG1, 1, &data[0]);
	log_debug("config_int1 read INT_SOURCE6_MREG1 0x%02x %02x\r\n", data[0], data[1]);
	...
	log_debug("config_int1 write INT_SOURCE6_MREG1 0x%02x %02x\r\n", data[0], data[1]);
...
int inv_imu_get_data_from_fifo(struct inv_imu_device *s)
{
	...
	/* Ensure data ready status bit is set */
	if ((status |= inv_imu_read_reg(s, INV_INT_STATUS, 1, &int_status)))
		return status;
if (int_status & INT_STATUS_FIFO_FULL_INT_MASK) {
    status |= inv_imu_reset_fifo(s);
    return status ? status : INV_ERROR;
}
	...
	if ((int_status & INT_STATUS_FIFO_THS_INT_MASK) ||
		...
		if (packet_count > (FIFO_MIRRORING_SIZE / packet_size)) {
		...
		while (packet_count > 0) {
			...
			uint32_t total_bytes = (uint32_t)packet_size * packet_count;
			uint32_t offset = 0U;
			uint32_t max_chunk = (SPI_MAX_READ_PAYLOAD / packet_size) * packet_size;

			while (offset < total_bytes && max_chunk != 0U) {
				uint32_t chunk = total_bytes - offset;
				if (chunk > max_chunk) {
					chunk = max_chunk;
				}

				if (inv_imu_read_reg(s, FIFO_DATA, chunk, &s->fifo_data[offset]) != INV_ERROR_SUCCESS) {
					status |= INV_ERROR;
					break;
				}
				offset += chunk;
			...
			if (max_chunk == 0U || status != INV_ERROR_SUCCESS) {
				/* 
				...
				if (header->Byte == 0x80) {
					...
					if (header->bits.gyro_bit && ((event.gyro[0] != INVALID_VALUE_FIFO) &&
						...
						if (s->gyro_start_time_us == UINT32_MAX) {
							...
							if (!header->bits.fsync_bit &&
							    ((inv_imu_get_time_ms() - s->gyro_start_time_us) >=
							     GYR_STARTUP_TIME_US)) {
								/* Discard first data after startup to let output to settle */
								s->gyro_start_time_us = UINT32_MAX;
								event.sensor_mask |= (1 << INV_SENSOR_GYRO);
...
static int init_hardware_from_ui(struct inv_imu_device *s)
{
	...
	log_debug("INT_CONFIG value1 = 0x%02x\r\n", value); 
	...
	status |= inv_imu_write_reg(s, INT_CONFIG, 1, &value);
	log_debug("value2 = 0x%02x\r\n", value);
	
	status |= config_wom_int1(); //llx add
...
//		inv_imu_sleep_us(100);
	...
	} while (inv_imu_get_time_ms() - start < 50); //(inv_imu_get_time_us() - start < 50000);
