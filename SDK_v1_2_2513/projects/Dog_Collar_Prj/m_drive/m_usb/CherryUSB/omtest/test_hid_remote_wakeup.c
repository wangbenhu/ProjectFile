...
static volatile bool hid_is_configured = false;
static volatile bool hid_is_suspend = false;
static volatile bool hid_is_remote_wakeup_enabled = false;
...
static void usbd_event_handler(uint8_t busid, uint8_t event)
{
    switch (event) {
        ...
        case USBD_EVENT_RESUME:
            ...
            hid_is_suspend = false;
            if(xEvtEvent) osEventFlagsSet(xEvtEvent, EVENT_FLAG_HID_RESUME);
            break;
        case USBD_EVENT_SUSPEND:
            USB_LOG_RAW("**USBD_EVENT_SUSPEND\r\n");
            hid_is_suspend = true;
            if(xEvtEvent) osEventFlagsSet(xEvtEvent, EVENT_FLAG_HID_SUSPEND);
            break;
        case USBD_EVENT_CONFIGURED:
            USB_LOG_RAW("**USBD_EVENT_CONFIGURED\r\n");
            hid_state = HID_STATE_IDLE;
            hid_is_suspend = false;
            hid_is_configured = true;
            break;
        case USBD_EVENT_SET_REMOTE_WAKEUP:
            USB_LOG_RAW("**USBD_EVENT_SET_REMOTE_WAKEUP\r\n");
            hid_is_remote_wakeup_enabled = true;
            break;
        case USBD_EVENT_CLR_REMOTE_WAKEUP:
            USB_LOG_RAW("**USBD_EVENT_CLR_REMOTE_WAKEUP\r\n");
            hid_is_remote_wakeup_enabled = false;
            break;
