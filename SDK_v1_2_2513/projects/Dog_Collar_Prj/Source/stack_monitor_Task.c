...
static osSemaphoreId_t g_StackMonitorSemaphore = NULL;//低功耗是否进入的标志
static bool stack_monitor_stop_flag = false;//false/true start/stop
/*********************************************************************
 * LOCAL FUNCTIONS
 *********************************************************************/


void stack_monitor_run_flag_set(void)
{
	stack_monitor_stop_flag = true;
}
bool stack_monitor_run_flag_get(void)
{
	return stack_monitor_stop_flag;
}
void stack_monitor_run_flag_clear(void)
{
	stack_monitor_stop_flag = false;
}
/**
...
{
    (void)argument;
	stack_monitor_run_flag_clear();
    /* 上电后稍等系统稳定 */
    osDelay(3000);
	if (g_StackMonitorSemaphore == NULL) {
		g_StackMonitorSemaphore = osSemaphoreNew(1, 0, NULL);
    } else {
        log_debug("vSensorTask already exists, skip creation\r\n");
    }
    for (;;)
    {
		osSemaphoreRelease(g_StackMonitorSemaphore);
		
        PrintStackReport();
        osDelay(STACK_MONITOR_INTERVAL_MS);
		if(stack_monitor_run_flag_get())
		{
			osSemaphoreAcquire(g_StackMonitorSemaphore, osWaitForever);
		}
