...
COMM_GET_STATUS_t commCurrModeStatus = {
	...
	.currentChargeStatus =CURRENT_CHARGE_STATUS_INVALID,
    .BAT_Capacity = 0,
};
/* RTOS对象句柄 */
...
UserBoundState_t User_BoundState = USER_BOUND_STATE_UNKNOWN;
...
static struct 
{
	...
	uint32_t user_define_light_start_time;
}user_define_light={0,0};
//static uint8_t user_define_light_start_flag = 0;//用户自定义灯光开启标志0/1 关/开
/*********************************************************************
 * GLOBAL VARIABLES
 */
...
extern uint8_t cat1_task_restart(TASK_ID_T source_id);
// 使用现有的函数组合实现
extern  om_error_t drv_oflash_deep_power_down(OM_OSPI_Type *om_flash);
...
uint8_t user_define_light_start_flag_get(void)
{
	return user_define_light.user_define_light_start_flag;
}
uint32_t user_define_light_start_time_get(void)
{
	return user_define_light.user_define_light_start_time;
}
void user_define_light_start_flag_set(uint32_t time)
{
	user_define_light.user_define_light_start_flag = 1;
	user_define_light.user_define_light_start_time = time;
}
void user_define_light_start_flag_clear(void)
{
	if(user_define_light_start_flag_get() ==1)
	{
		user_define_light.user_define_light_start_flag = 0;
		user_define_light.user_define_light_start_time = 0;
	}
}
//0未绑定1成功
void uset_bound_state_set(UserBoundState_t state)
{
	User_BoundState = state;
}
/*
* @brief: 获取user_bound_flag状态
* @param: void
* @return: uint8_t: user_bound_flag状态
*/
UserBoundState_t uset_bound_state_get(void)
{
	return User_BoundState;
}

/*
* @brief: 获取product_entry_flag状态
* @param: void
* @return: uint8_t: product_entry_flag状态
*/
...
* @brief: 获取当前充电状态数据
* @param: void
* @return: CURRENT_CHARGE_STATUS_T: 当前充电状态数据
*/
CURRENT_CHARGE_STATUS_T CurrentChargeStatusDataGet(void)
{
    return commCurrModeStatus.currentChargeStatus;
}
/*
* @brief: 设置当前充电状态数据
* @param: charge_status: 当前充电状态数据
* @return: void
*/
void CurrentChargeStatusDataSet(CURRENT_CHARGE_STATUS_T charge_status)
{
   commCurrModeStatus.currentChargeStatus = charge_status;
}
...
void COMM_MODE_REPORT(SystemMode_t mode)
{
    ...
    {
        case MODE_M1:
            CurrentModeDataSet(CURRENT_MODE_DEFAULT);
			CurrentChargeStatusDataSet(CURRENT_CHARGE_STATUS_INVALID);
            break;
        case MODE_M2:
			 CurrentModeDataSet(CURRENT_MODE_STANDARD);
			CurrentChargeStatusDataSet(CURRENT_CHARGE_STATUS_LOW_BATTERY);
            break;
        case MODE_M3:
			 CurrentModeDataSet(CURRENT_MODE_STANDARD);
			CurrentChargeStatusDataSet(CURRENT_CHARGE_STATUS_LOW_BATTERY);
        ...
        case MODE_M4:
			CurrentModeDataSet(CURRENT_MODE_STANDARD);
			CurrentChargeStatusDataSet(CURRENT_CHARGE_STATUS_CHARGING);
            break;
        case MODE_M5:
			CurrentModeDataSet(CURRENT_MODE_STANDARD);
			CurrentChargeStatusDataSet(CURRENT_CHARGE_STATUS_FULL);
            break;
        default:
            CurrentModeDataSet(CURRENT_MODE_DEFAULT);
			CurrentChargeStatusDataSet(CURRENT_CHARGE_STATUS_INVALID);
...
static int system_sync_init(void)
{
    ...
    if (g_sleepEntryReadyFlags == NULL)
    {
//        log_debug("Failed to create sleep entry event flags!\r\n");
        return -1;
    }
...
{
	s_Charge_Stop_Flag = true;
    osSemaphoreRelease(xSemAgingTest);//释放entry 退出阻塞
}
//拔掉充电状态获取
bool Entry_Control_Stop_Power_Charge_FlagGet(void)
{
...
//			red_led_start(R_LED_STATE_FINDING_DEVICE);//常量
	//		//修改后常量20260417需求更改增加用户绑定取消灯亮
			//log_debug("uset_bound_state_get() = %d %d\r\n",uset_bound_state_get(),GetUser_ActivatedState());
			 UserBoundState_t lfs_state = uset_bound_state_get();
			if(lfs_state == USER_BOUND_STATE_UNKNOWN)//如果没有更新状态，从文件系统读取
			{
			...
			else if(lfs_state == USER_BOUND_STATE_UNBOUND)
			{
				red_led_start(R_LED_STATE_FINDING_DEVICE);
			}
			else
	...
	//注释了M3模式的低电量显示和M3之后的寻宠模式灯效关闭，在M3进入接口统一关闭所有LED
		...
		else
		{
			if(user_define_light_start_flag_get() == 1)
			{
				white_led_start(W_LED_STATE_USER,user_define_light_start_time_get());
			}
			else
			{
				...
				else if(CurrentModeDataGet() == CURRENT_MODE_STANDARD)
				{
					white_led_status_reset();
				}
				else
...
void Entry_Init(void)
{
    ...
    SetModePare_SetUser(GetUser_ActivatedState());//设置用户信息
...
//  	TaskInfo_InitTask(task_info,AUDIO_MIC_TASK_ID,QUEUE_LENGTH,MESSAGE_SIZE,vStartAudioMicTask);
	 //获取当前初始化的全部task信息
	get_task_state();
}
...
static osStatus_t EntryTask_HandleMessageQueue(TaskInfo_t* pEntryTaskInfo, Message_t* received_msg)
{
    ...
    if(osOK == status)
    {
        ...
        {
//			log_batt_debug("product_flag_get TASK_INFO_CHARGE!\r\n");
            BAT_STATUS_t tmp;
        ...
        if(received_msg->source_id == COMM_TASK_ID)
        {
			...
			if(received_msg->command == TASK_LED_USER_START)   	// 用户灯光开启
			{
//				log_debug("TASK_LED_USER_START = %d \r\n",*(uint32_t *)received_msg->data);
				user_define_light_start_flag_set(*(uint32_t *)received_msg->data);
			}
			if(received_msg->command == TASK_LED_USER_STOP)//用户灯光关闭
			{
//				log_debug("TASK_LED_USER_STOP\r\n");
				user_define_light_start_flag_clear();
			}
        }
...
//        log_debug("Semaphore already exists, skip creation\r\n");
    ...
    while(1)
    {
        if(Entry_Control_Stop_Power_Charge_FlagGet()){
            //拔掉充电器后处理
            Entry_Control_Stop_Power_Charge();//卸载文件系统后重启
        }
//更新电量百分比
