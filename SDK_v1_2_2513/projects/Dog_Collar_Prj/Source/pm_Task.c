...
// #define DISCHG_DEADBAND_MV                12    // 放电死区
// #define DISCHG_FALL_MAX_MV_HIGH              6   // 10s 内最大下降
// #define DISCHG_FALL_MAX_MV_AVG               5    // 10s 内最大下降
// #define DISCHG_FALL_MAX_MV_LOW               4    // 10s 内最大下降
...
/* ===== 内部状态 ===== */
#define BATTERY_CAPACITY_CONFIG 0//0/1 1适配所有电池容量，0只适用1500mAh
static int32_t batt_mv_filt = 0;
static uint8_t batt_init = 0;
static uint8_t initialized = 0;  // 初始化标志

static uint8_t power_on_bertty_vlue = 0;  // 初始化标志

static uint8_t battery_full_flag = 0;  

#if (BATTERY_CAPACITY_CONFIG)
	 typedef enum {
		BATT_R_LOW = 0,
		BATT_R_MID,
		BATT_R_HIGH
	...
	static batt_cap_t batt_cap = BATT_CAP_300;
	typedef struct {
		uint8_t iir_shift;      // 0=不过滤，1=1/2，2=1/4
		int32_t fall_max_mv;    // 最大下降
		int32_t rise_max_mv;
	} batt_filter_profile_t;
	static const batt_filter_profile_t batt_profile[][3] = {
		/* 内阻: LOW_R */
		{
			/* 1500 */ {2, 15, 12},
			/* 500  */ {1, 20, 15},
...
//		initialized = 1;
//        return soc_now;
		 uint8_t power_on_soc = power_on_bertty_vlue;
                                ...
                                CHARGE_STATUS_T charging)
{
    ...
    if (!batt_init)
    {
//		uint32_t vbat_tmp=PercentToVoltage((uint8_t)drv_pmu_retention_reg2_get());
//		log_debug(" BatteryVoltage_AntiJump batt_init:%d\r\n",PercentToVoltage((uint8_t)drv_pmu_retention_reg2_get()));
//		log_aging_debug(" BatteryVoltage_AntiJump batt_init:%d %d\r\n",PercentToVoltage((uint8_t)drv_pmu_retention_reg2_get()),new_mv);
//		if(vbat_tmp!=new_mv &&vbat_tmp - new_mv<=500 && vbat_tmp!=0)
//		{
//			batt_mv_filt=vbat_tmp;
//		}
//		else
			batt_mv_filt = new_mv;
        batt_init = 1;
...
{
//	log_debug("-----g_PMBatStatus.BAT_Capacity = %d---\r\n",g_PMBatStatus.BAT_Capacity);
...
{
//	log_debug("-----g_PMBatStatus.BAT_Capacity = %d---\r\n",g_PMBatStatus.BAT_Capacity);
//	g_PMBatStatus.BAT_Capacity = VoltageToPercent(voltage_mV);
...
{
	uint32_t get_voltage_mv_tmp = 0;
	int32_t voltage_mv_filter = 0;
	uint8_t full;
	get_voltage_mv_tmp = adc_to_mv_generic(adv_vlue);//原始数据转换MV
	voltage_mv_filter = BatteryVoltage_AntiJump(get_voltage_mv_tmp,PM_GetChargeStatus());//原始电压滤波处理
	
	g_PMBatStatus.BAT_Capacity = Battery_SocUpdate(voltage_mv_filter,PM_GetChargeStatus());//电压转换百分比
	reset_rom_battery_percent(g_PMBatStatus.BAT_Capacity);
	log_batt_debug("battery_voltage_state_update = ADC[%d] Percentage[%d] filter[%d]\r\n",adv_vlue,g_PMBatStatus.BAT_Capacity,voltage_mv_filter);
	log_debug("battery_voltage_state_update = ADC[%d] Percentage[%d] filter[%d]\r\n",adv_vlue,g_PMBatStatus.BAT_Capacity,voltage_mv_filter);
...
void PM_Charge_Battery_Init(void)
{
	...
	power_on_bertty_vlue = drv_pmu_retention_reg2_get();//获取最后一次充电的百分比数据
	drv_pmu_retention_reg2_set(0);
	get_voltage_mv_tmp = adc_to_mv_generic(r_PowerOn_BatteryLevel());//原始数据转换MV

	voltage_mv_filter = BatteryVoltage_AntiJump(get_voltage_mv_tmp,PM_GetChargeStatus());//原始电压滤波处理
//	g_PMBatStatus.BAT_Capacity = VoltageToPercent(voltage_mv_filter);//电压转换百分比
	
	g_PMBatStatus.BAT_Capacity = Battery_SocUpdate(voltage_mv_filter,PM_GetChargeStatus());//电压转换百分比
	
	full = Battery_FullDetect(voltage_mv_filter,PM_GetChargeStatus());//判断是否充满
	if (full)
    {
		g_PMBatStatus.BAT_Capacity=100;
        // 充满逻辑
//		if(g_PMBatStatus.charge_status == CHARGE_STATUS_CHARGING)
//			g_PMBatStatus.charge_status = CHARGE_STATUS_FULL;
    }
	
	
	reset_rom_battery_percent(g_PMBatStatus.BAT_Capacity);
	log_batt_debug("PM_Charge_Battery_Init = ADC[%d] state[%d] Percentage[%d] %d\r\n",get_voltage_mv_tmp,PM_GetChargeStatus(),g_PMBatStatus.BAT_Capacity,voltage_mv_filter);
	
	log_debug("PM_Charge_Battery_Init = ADC[%d] state[%d] Percentage[%d] %d\r\n",get_voltage_mv_tmp,PM_GetChargeStatus(),g_PMBatStatus.BAT_Capacity,voltage_mv_filter);
}
...
void PM_battery_timeout(void)
{
    ...
    //measure battery voltage and charge status
...
BAT_STATUS_t* GetPMBatObj()
{
    return &g_PMBatStatus;
}
