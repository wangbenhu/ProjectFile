...
osTimerId_t Asing_Test_Stop_Ble_Timer_ID = NULL;
...
osTimerAttr_t Asing_Test_Stop_Ble_Timer_attr = {
	.name = "Asing_Test_Stop_Ble_Timer",
};
 ...
 */
 ...
 /*
 * */
 //asing_stop_ble_flag设置
void SetAsingStopBleFlag(ASING_STOP_BLE_FLAG_T flag)
{
	asing_stop_ble_flag = flag;
}
/*
* asing_stop_ble_flag获取
* @return  asing_stop_ble_flag
*/
ASING_STOP_BLE_FLAG_T GetAsingStopBleFlag(void)
{
	return asing_stop_ble_flag;
}
/**
...
void TestTimerCallback(void *argument)
{
	...
	switch(asing_mode_entry_flag)//进入测试模式
	{
		...
		case 2://关灯 关电机 开喇叭
			RepetFlagSet();//设置重�?�播�?
			Audio_Play_Request("/audio/player/p1.wav",1);
 ...
 * @return  无
 **/
void Stop_Asing_Test_Stop_Ble_Timer(void)
{
	if(Asing_Test_Stop_Ble_Timer_ID)
	{
		osTimerStop(Asing_Test_Stop_Ble_Timer_ID);   // 
        osTimerDelete(Asing_Test_Stop_Ble_Timer_ID); // 
        Asing_Test_Stop_Ble_Timer_ID = NULL;          // 
	}
}
void DelayStopBleTimerCallback(void *argument)
{
	#if(0)
	if(GetAsingStopBleFlag() == ASING_STOP_BLE_FLAG_END)
	{
		evt_app_adv_stop();//关闭蓝牙
		//关闭蓝牙定时器
		Stop_Asing_Test_Stop_Ble_Timer();
		//设置蓝牙状态为默认
...
void StopBleRestartTimer(void) {
#if(0)
 	Stop_Asing_Test_Stop_Ble_Timer();
    Asing_Test_Stop_Ble_Timer_ID = osTimerNew(DelayStopBleTimerCallback, osTimerPeriodic, NULL, &Asing_Test_Stop_Ble_Timer_attr);
	osTimerStart(Asing_Test_Stop_Ble_Timer_ID, osMS2TicksRound(TEST_AGING_STOP_BLE_DELAY_TIME));
#else
	 Stop_Asing_Test_Stop_Ble_Timer();
    Asing_Test_Stop_Ble_Timer_ID = osTimerNew(DelayStopBleTimerCallback, osTimerOnce, NULL, &Asing_Test_Stop_Ble_Timer_attr);
	osTimerStart(Asing_Test_Stop_Ble_Timer_ID, osMS2TicksRound(TEST_AGING_STOP_BLE_DELAY_TIME));
	#endif
} 
...
//				log_debug("EventFlags error: %#x\r\n", os_event_flags);
		...
		{
			log_debug("PRODUCTION_TASK_EXAMPLE = %08x %d\r\n",os_event_flags,test_mutual_flag);
			...
			if(os_event_flags & PRODUCTION_TASK_EXAMPLE_16)//老化测试
			{
				...
				else
				{
					...
					//	lfs_list_dir("/");
						...
						RestartTimer();
						SetAsingStopBleFlag(ASING_STOP_BLE_FLAG_START);
						//StopBleRestartTimer();
						error_code_tmp = REPORT_OK;
						osEventFlagsSet(g_Production_Task_Flags, PRODUCTION_TASK_EXAMPLE_REPORT);
					}
...
//					if(adc_vlue_tmp > (TEST_ADC_ELECTRIC+TEST_ADC_ELECTRIC_OFFSET) || adc_vlue_tmp < (TEST_ADC_ELECTRIC-TEST_ADC_ELECTRIC_OFFSET))
			...
			{
				log_debug("PRODUCTION_TASK_EXAMPLE_REPORT \r\n");
