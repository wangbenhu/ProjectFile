 ...
 *  - 协议帧: 状态(1) + 数据类型(1) + 传输ID(1) + 总长度(4) + CRC(2) = 9 字节
...
#define BULK_MTU_SIZE               160    // 固定 MTU
#define BULK_PACKETS_PER_ROUND      10     // 每轮 10 包
#define BULK_BUFFER_SIZE            2048   // 2KB 累积缓冲
...
/* 开始包: 9 字节 = 状态(1)+类型(1)+传输ID(1)+总长度(4)+CRC(2) — 加 CRC 校验 */
static void bulk_process_start(const uint8_t *data, uint16_t len) {
    if (len != 9) {
        log_debug("[BULK] start bad len=%d (expect 9)\r\n", len);
    ...
    uint32_t total_length = ((uint32_t)data[3] << 24) | ((uint32_t)data[4] << 16) |
                            ((uint32_t)data[5] << 8)  |  (uint32_t)data[6];
    uint16_t crc_exp = (uint16_t)((data[7] << 8) | data[8]);
    /* CRC 校验 header (7字节: 状态+类型+传输ID+总长度) */
    uint16_t crc_cal = bulk_calculate_crc(data, 7, 0xFFFFU);
    if (crc_cal != crc_exp) {
...
/* 数据包: 160 字节固定, 累积, 每 10 包或 buffer 满 ACK */
static void bulk_process_data(const uint8_t *data, uint16_t len) {
    if (m_bulk.state != M_BULK_STATE_TRANSFERRING) {
    ...
    if (m_bulk.buffer_offset + len > BULK_BUFFER_SIZE) {
        log_debug("[BULK] data buf overflow: off=%d + len=%d > %d\r\n",
                  m_bulk.buffer_offset, len, BULK_BUFFER_SIZE);
    ...
    /* 条件1: buffer 满 */
    if (m_bulk.buffer_offset >= BULK_BUFFER_SIZE) {
        if (bulk_flush_buffer() != 0) {
            bulk_send_ack(BULK_CMD_NACK, m_bulk.bulk_type, m_bulk.transfer_id);
            return;
        }
