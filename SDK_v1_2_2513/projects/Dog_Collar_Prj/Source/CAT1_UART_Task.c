...
//            }
	...
	//		sprintf(mqtt_payload_str, "AT+QWIFISCAN=%d,%d,%d,%d,%d,%d\r\n", WIFISCAN_TIME, WIFISCAN_ROUND, WIFISCAN_NUM, WIFISCAN_TIMEOUT, WIFISCAN_PRIORITY,WIFISCAN_SSID_TYPE);
			sprintf(mqtt_payload_str, "AT+QWIFISCAN=12000,1,4,5,0,1\r\n");
			mqtt_payload_len = strlen(mqtt_payload_str);
			break;
		//http GET实时音频数据
...
BaseType_t mqtt_publish(unsigned char *data, unsigned int len)
{
	...
	{
		if (build_report_topic(topic, device_sn, report_topic, sizeof(report_topic)) < 0) {
			goto PUBLISH_FAIL;
		}
	}
    ...
    else
    {
        *status = CMD_STATUS_FAILED;
		...
		if (isMqttConnected) 
		{
			...
			// 网络已连接
			if(currentCmd->type == LTE_MQTT_CONN)
			{
				lteSubTopic();
			}
		}
...
void at_qmtpubex(uint8_t *str, uint16_t len, cmd_status_t *status)
{
	...
	if (sscanf(params, "+QMTPUBEX: %*d,%*d,%d", &result_code) >= 1) {
		if (result_code == 0) {
...
/**
 * @brief  at_wifiScan		cat1 wifi scan
 * @param str 数据    
 * @param len 数据长度
 * @param status 指令执行状态
 * @return NULL
...
void at_wifiScan(uint8_t *data, uint16_t len, cmd_status_t *status)
{
    ...
    static DeviceWifiSsid_t wifi_scan_result = {0};
    
    // 重置WiFi扫描结果
    memset(&wifi_scan_result, 0, sizeof(DeviceWifiSsid_t));
    wifi_scan_result.wifi_count = 0;
    
    // 检查是否有TIMEOUT错误
    if (strstr((char *)data, "TIMEOUT") != NULL)
    {
    ...
    // 循环处理所有WiFi条目（现在数据包含完整的+QWIFISCAN:前缀）
    while (current_pos < scan_data + len && wifi_scan_result.wifi_count < 10)
    {
        // 查找下一个+QWIFISCAN开始位置
        ...
        // 查找当前条目的结束位置（括号结束、下一个+QWIFISCAN或行结束）
        ...
        if (parse_single_wifi_info(data_start, data_end - data_start, 
                                  &wifi_scan_result.wifi_list[wifi_scan_result.wifi_count]))
        {
            wifi_scan_result.wifi_count++;
            found_entries = 1;
        }
    ...
    // 处理扫描结果状态
    if (wifi_scan_result.wifi_count > 0)
    {
        send_wifi_scan_data_to_comm_task(&wifi_scan_result);
        *status = CMD_STATUS_SUCCESS;
    }
    else if (found_entries == 0 && strstr((char *)data, "OK") != NULL)
    {
        // 扫描完成但没有找到网络
        send_empty_wifi_scan_result();
        *status = CMD_STATUS_FAILED;
    ...
    {
        // 扫描失败
        log_debug("[CAT1][ERR] WiFi scan failed\r\n");
        send_empty_wifi_scan_result();
        *status = CMD_STATUS_FAILED;
    }
}
...
//					start_delta_firmware("http://4c5966cd.r10.cpolar.top/download/signed_EG800QEULCR01A11M04_A0.300.A0.300-EG800QEULCR01A11M04_BETA2025071002.par", 119);
	...
	{
		log_debug("[CAT1][STA] CONNECT response detected, len=%d\r\n", len);
		if (currentCmd != NULL)
		{
...
//        log_debug("URC data processed, urc_status=%d\n", urc_status);
//		urc_processed = true;
//        //继续检查命令响应
    ...
    if (currentCmd && (currentCmd->type == LTE_MQTT_PUB || currentCmd->type == LTE_MQTT_PUBMESSAGEDATA))
    {
        ...
        if (has_ack && currentCmd->type == LTE_MQTT_PUB && 
            ...
            log_debug("[CAT1][STA] MQTT PUB > + OK bundled, pre-clearing g_mqtt_sending\r\n");
            g_mqtt_sending = 0;
            mqtt_send_start_tick = 0;
            g_current_send_type = 0;
...
/**
 * @brief 设置主动上报模式
 * @param mode 上报模式：1-常规模式(1分钟) 2-寻宠模式(3秒)
 */
void set_auto_report_mode(Message_t *msg)
{
	AppControlMode_t *mode_data = (AppControlMode_t *)msg->data;
    if (mode_data->mode == MODE_STANDARD) {
        current_report_mode = REPORT_MODE_STANDARD;
        report_interval_ms = 10 * 60 * 1000;
        log_debug("[CAT1][STA] Set auto report: STANDARD (10min interval)\r\n");
    } else if (mode_data->mode == MODE_SEARCH_PET) {
        current_report_mode = REPORT_MODE_SEARCH;
        report_interval_ms = 3 * 1000;
        log_debug("[CAT1][STA] Set auto report: SEARCH (3s interval)\r\n");
    } else {
        current_report_mode = REPORT_MODE_NONE;
    }
    
    // 重启定时器
//    if (autoReportTimer_ID && isMqttConnected) {
//        if (osTimerIsRunning(autoReportTimer_ID)) {
//            osTimerStop(autoReportTimer_ID);
//        }
//        if (current_report_mode != REPORT_MODE_NONE) {
//            osTimerStart(autoReportTimer_ID, report_interval_ms);
//            log_debug("Auto report timer restarted with interval %d ms\n", report_interval_ms);
//        }
//    }
}

/**
 * @brief 主动上报定时器回调 — 已迁移到COMM任务
 * @note  此函数已弃用，主动上报逻辑由COMM的CommAutoReportTimerCallback接管
 *        保留空函数体防止链接错误（定时器创建代码也已注释掉）
 */
void TimerCallback_AutoReport(void *argument)
{
    // 主动上报已迁移到COMM任务，此回调不再执行任何逻辑
    log_debug("[CAT1][STA] TimerCallback_AutoReport deprecated, handled by COMM\r\n");
}
...
static void vCAT1UartTask(void *argument)
{
	...
	// autoReportTimer_ID = osTimerNew(TimerCallback_AutoReport, autoReportTimer_type, NULL, &autoReportTimer_attr);
	// if (!autoReportTimer_ID)
	// 	LOG_LOC();
...
//				else if (cat1_receive_msg.command == TASK_DATA_VAL_UPDATA)
//				{
//					flag_check_device_status = true;
					// 信号质量数据更新
//					lteUpdataCsq();
//				}
//				else if (cat1_receive_msg.command == TASK_CAT1_QUERY_ENODEB)
//				{
//					// 查询基站消息
//					CELL_QueryCellInfo();
//				}
//				else if (cat1_receive_msg.command == TASK_CAT1_DEVICE_STATE_REPORT)
//				{
//					// 设置模式
////					set_auto_report_mode(&cat1_receive_msg);
//				}
...
//                            osEventFlagsSet(LteEventId, LTE_EVENT_ERROR);
		...
		if (currentCmd && (osKernelGetTickCount() - LastWakeTime) > osMS2TicksRound(CAT1_COMMAND_TIMEOUT))
		{
